...
public class Solve1 extends Thread {
	...
	public Solve1(String url1, String url2, String url3, String url4, boolean AA, boolean Aa, boolean AN, boolean aA,
	...
	public void run() {
		driver.get(url1);
		if (AA || Aa || AN) {
			outerloop: for (char chA = 'A'; chA <= 'F'; chA++) {
				if (Aa) {
							...
							try {
								if (!cookieAccepted) {
									WebElement input = wait.until(ExpectedConditions.visibilityOfElementLocated(By
											.xpath("//*[@id=\"bodyel\"]/section[4]/div[10]/section/div/div[2]/div/input")));
									input.clear();
									input.sendKeys(url2 + Character.toString(chA) + cha+ url3);
									driver.findElement(
											By.xpath(("//*[@id=\"bodyel\"]/section[4]/div[10]/footer/div/button")))
											.click();
									System.out.print(Character.toString(chA) + cha + ",");
									wait.until(ExpectedConditions.visibilityOfElementLocated(
											By.xpath("//*[@id=\"bodyel\"]/section[1]/div[4]/div[1]/div[2]/button[1]")))
											.click();
									cookieAccepted = true;
		...
		if (NA || Na || NN) {
			outerloop: for (int chA = 0; chA <= 1; chA++) {
				if (Na) {
					for (char cha = 'a'; cha <= 'z'; cha++) {
						if (Main.getFound() == false) {
