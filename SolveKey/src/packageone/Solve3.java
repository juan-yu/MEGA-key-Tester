...
public class Solve3 extends Thread {
	...
	public Solve3(String url1, String url2, String url3, String url4, boolean AA, boolean Aa, boolean AN, boolean aA,
	...
	public void run() {
		driver.get(url1);
		if (AA || Aa || AN) {
			outerloop: for (char chA = 'L'; chA <= 'P'; chA++) {
				if (Aa) {
		...
		if (NA || Na || NN) {
			outerloop: for (int chA = 4; chA <= 5; chA++) {
				if (Na) {
					for (char cha = 'a'; cha <= 'z'; cha++) {
						if (Main.getFound() == false) {
