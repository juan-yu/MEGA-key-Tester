...
public class Solve4 extends Thread {
	...
	public Solve4(String url1, String url2, String url3, String url4, boolean AA, boolean Aa, boolean AN, boolean aA,
	...
	public void run() {
		driver.get(url1);
		if (AA || Aa || AN) {
			outerloop: for (char chA = 'Q'; chA <= 'U'; chA++) {
				if (Aa) {
		...
		if (NA || Na || NN) {
			outerloop: for (int chA = 6; chA <= 7; chA++) {
				if (Na) {
					for (char cha = 'a'; cha <= 'z'; cha++) {
						if (Main.getFound() == false) {
