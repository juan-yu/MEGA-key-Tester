...
public class Solve5 extends Thread {
	...
	public Solve5(String url1, String url2, String url3, String url4, boolean AA, boolean Aa, boolean AN, boolean aA,
	...
	public void run() {
		driver.get(url1);
		if (AA || Aa || AN) {
			outerloop: for (char chA = 'V'; chA <= 'Z'; chA++) {
				if (Aa) {
		...
		if (NA || Na || NN) {
			outerloop: for (int chA = 8; chA <= 9; chA++) {
				if (Na) {
					for (char cha = 'a'; cha <= 'z'; cha++) {
						if (Main.getFound() == false) {
